...
export default function UserOverview() {
  ...
  useEffect(() => {
    ...
    const fetchUserResults = async () => {
      try {
        ...
        setPercentage(
          ((snapshot.receivedPoints / snapshot.quizPoints) * 100).toFixed(2)
        );
      } catch (error) {
  ...
  useEffect(() => {
    const unblock = history.block((tx) => {
      if (tx.action === 'POP') {
        const allowNavigation = window.confirm("Are you sure you want to leave this page?");
        if (!allowNavigation) {
          history.go(1);
        }
      }
            ...
            answers.map((answer, index) => {
              ...
              return (
                <div key={index}>
                  <p>Question: {question.question}</p>
                  <p style={{ color: isCorrect ? "green" : "red" }}>
                    Your answer: {selectedAnswer.split("-")[0]}{" "}
                    {isCorrect ? "✅" : "❌"}
                  </p>
                  {isCorrect ? null : (
                    <p>
